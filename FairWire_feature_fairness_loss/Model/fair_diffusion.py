...
class ModelSync(BaseModel):
    ...
    def sample(self,
        ...
        if playback_controller_replay:
            ...
            if X_t_one_hot.dim() == 2:
                edge_node_features = X_t_one_hot.reshape(
                    self.num_nodes, self.num_attrs_X, self.num_classes_X)[..., 1]
            elif X_t_one_hot.dim() == 3:
                edge_node_features = X_t_one_hot[..., 1].transpose(0, 1)
                X_t_one_hot = X_t_one_hot.transpose(0, 1).reshape(self.num_nodes, -1)
            else:
