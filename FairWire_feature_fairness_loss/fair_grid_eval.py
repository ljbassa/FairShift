...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--skip_existing",
        action="store_true",
        help="Reuse existing generated .pyg.pt files and evaluation summaries when present.",
    )
    p.add_argument(
        "--plot_x_metric",
        type=str,
...
def ordered_fieldnames(rows: List[Dict]) -> List[str]:
    preferred = [
        ...
        "overlap/auc_mean",
        "lp/auc_mean_std",
        "lp/sp_abs_gap_mean_std",
        "fair_abs_gap_mean_std",
        "value/fair_abs_gap_mean_std",
        "value/linkpred_auc_mean_std",
        "lp/score_sp_abs_gap_mean_std",
        "overlap/auc_mean_std",
        "raw_log",
        "sample_cmd",
        "eval_cmd",
        "pt_path",
...
def metric_aliases(metric: str) -> List[str]:
    ...
    if metric in {
        "sp_score_abs_gap",
        "sp_score_abs_gap_mean",
        "score_sp_abs_gap",
        "score_sp_abs_gap_mean",
        "lp/sp_score_abs_gap",
        "lp/sp_score_abs_gap_mean",
        "lp/score_sp_abs_gap",
    }:
        aliases.append("lp/score_sp_abs_gap_mean")
    return aliases


def resolve_metric_key(rows: List[Dict], metric: str) -> str:
    candidates = [metric, *metric_aliases(metric), f"{metric}_seed_mean"]
    if not metric.endswith("_mean"):
        candidates.append(f"{metric}_mean")
...
def resolve_std_key(rows: List[Dict], metric_key: str) -> str:
    candidates = [f"{metric_key}_std", f"{metric_key}_seed_std"]
    if metric_key.endswith("_seed_mean"):
        candidates.insert(0, f"{metric_key[:-len('_seed_mean')]}_seed_std")
    for key in candidates:
        if any(is_number(row.get(key)) for row in rows):
            return key
...
def plot_pareto(
    ...
    x_key = resolve_metric_key(agg_rows, x_metric)
    x_std_key = resolve_std_key(agg_rows, x_key)
    y_key = resolve_metric_key(agg_rows, y_metric)
    y_std_key = resolve_std_key(agg_rows, y_key)

    valid = [
        row for row in agg_rows
        if is_number(row.get(x_key)) and is_number(row.get(y_key)) and row.get("n_success", 0) > 0
    ]
    if not valid:
        n_success = sum(int(row.get("n_success", 0) or 0) for row in agg_rows)
        raise RuntimeError(
            f"No aggregated rows contain {x_key} and {y_key} with successful evaluations "
            f"(total successful eval runs: {n_success}). Check raw_logs and summary_long.csv."
        )
    ...
    plt.xlabel(f"{x_key} (lower is better)")
    plt.ylabel(f"{y_key} (higher is better)")
    plt.title(title)
    plt.grid(True, alpha=0.3)
    plt.tight_layout()

    dataset_tag = sanitize_tag(dataset)
    png_path = out_dir / f"{dataset_tag}_pareto_curve.png"
    pdf_path = out_dir / f"{dataset_tag}_pareto_curve.pdf"
    plt.savefig(png_path, dpi=200)
    plt.savefig(pdf_path)
    plt.close()

    pareto_csv = out_dir / f"{dataset_tag}_pareto_front.csv"
...
def main() -> None:
    ...
    for model_path in model_paths:
        ...
        for seed in args.seeds:
            ...
            existing_pt = pt_path.exists()
            existing_summary = summary_csv.exists()
            sample_proc = None
            reused_sample = args.skip_existing and existing_pt
            if not reused_sample:
            ...
            row = {
                ...
                "model_path": str(model_path),
                "seed": seed,
                "sample_returncode": 0 if reused_sample else sample_proc.returncode,
                "reused_sample": reused_sample,
                "pt_path": str(pt_path),
            ...
            eval_proc = None
            reused_eval = False
            if row["sample_returncode"] == 0:
                reused_eval = args.skip_existing and reused_sample and existing_summary
                row["reused_eval"] = reused_eval
                if reused_eval:
                    try:
                        row.update(parse_summary_csv(summary_csv))
                    except Exception as e:
                        row["eval_error"] = f"Failed to parse summary CSV: {e}"
                    row["eval_returncode"] = 0 if "eval_error" not in row else -1
                else:
            ...
            with raw_path.open("w", encoding="utf-8") as f:
                ...
                f.write("[reused existing generated graph]\n" if reused_sample else sample_proc.stdout)
                f.write("\n\nSAMPLE STDERR:\n")
                f.write("" if reused_sample else sample_proc.stderr)
                f.write(f"\n\nSAMPLE RETURNCODE: {row['sample_returncode']}\n")
                f.write("\n\nEVAL CMD:\n")
                f.write(" ".join(eval_cmd) + "\n\n")
                if reused_eval:
                    f.write("EVAL STDOUT:\n")
                    f.write("[reused existing evaluation summary]\n")
                    f.write("\n\nEVAL STDERR:\n")
                    f.write(f"\n\nEVAL RETURNCODE: {row['eval_returncode']}\n")
