...
SAMPLEPY_DEVICE_OVERRIDE: Optional[str] = None


def set_samplepy_device(device: Optional[str]) -> None:
    global SAMPLEPY_DEVICE_OVERRIDE
    SAMPLEPY_DEVICE_OVERRIDE = device
...
def samplepy_device() -> torch.device:
    if SAMPLEPY_DEVICE_OVERRIDE:
        if str(SAMPLEPY_DEVICE_OVERRIDE).startswith("cuda") and not torch.cuda.is_available():
            return torch.device("cpu")
        return torch.device(SAMPLEPY_DEVICE_OVERRIDE)
...
def evaluate_graphs(
    ...
    *,
    ...
    set_samplepy_device(getattr(args, "device", None))
