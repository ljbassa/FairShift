...
SAMPLEPY_DEVICE: Optional[torch.device] = None


def set_samplepy_device(device: str) -> torch.device:
    global SAMPLEPY_DEVICE
    requested = torch.device(device)
    if requested.type == "cuda" and not torch.cuda.is_available():
        requested = torch.device("cpu")
    SAMPLEPY_DEVICE = requested
    return SAMPLEPY_DEVICE


def samplepy_device() -> torch.device:
    if SAMPLEPY_DEVICE is not None:
        return SAMPLEPY_DEVICE
...
def samplepy_config_list(args: Optional[argparse.Namespace] = None) -> List[Dict[str, Any]]:
    hyper_space = {
        "lr": getattr(args, "lp_search_lrs", [3e-2, 1e-2, 3e-3, 1e-3]),
        "num_layers": getattr(args, "lp_search_num_layers", [1]),
        "hidden_size": getattr(args, "lp_search_hidden_dims", [16, 32, 128, 512]),
        "dropout": getattr(args, "lp_search_dropouts", [0.0, 0.1, 0.2]),
    }
...
def samplepy_fit_trial(
    ...
    *,
    ...
    batch_size = max(1, int(lp_batch_size))
    num_nodes = Z.size(0)

    for epoch in range(1, int(lp_epochs) + 1):
        model.train()
        Z_out = model(A_train, Z)
...
def samplepy_train_and_eval(data, group_attr: str, args: Optional[argparse.Namespace] = None) -> Tuple[Dict[str, float], Dict[str, np.ndarray], Dict[str, float]]:
    data = ensure_features(data)
    ...
    for config in samplepy_config_list(args):
        trial_auc, _trial_sp, _trial_eo, trial_model, trial_meta = samplepy_fit_trial(
            ...
            split["train_mask"],
            split["val_mask"],
            lp_epochs=getattr(args, "lp_epochs", 1000),
            lp_patience=getattr(args, "lp_patience", 5),
            lp_batch_size=getattr(args, "lp_batch_size", 16384),
            **config,
        )
    ...
    *,
    ...
    if graph_index is not None:
        ...
        if max_graphs is not None:
            graphs = graphs[:max(1, int(max_graphs))]
    ...
    for i, data in enumerate(graphs):
        ...
        print(f"[eval] graph {i + 1}/{len(graphs)} (original_idx={original_idx})", flush=True)
        row: Dict[str, Any] = {
        ...
        try:
            lp_metrics, lp_raw, meta = samplepy_train_and_eval(data, group_attr=sensitive_attr, args=args)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max_graphs", type=int, default=None, help="Optional: evaluate only the first N graphs from the saved list")
...
def main() -> None:
    args = parse_args()
    eval_device = set_samplepy_device(args.device)
    print(f"[eval] device={eval_device}", flush=True)
    ...
    print(f"[eval] loaded_graphs={len(graphs)} graph_path={args.graph_path}", flush=True)
