...
def build_generate_cmd(args, eta: float, k: float, seed: int, save_dir: Path) -> List[str]:
    ...
    if generate_script.name == "sample.py":
        if args.model_path is None:
            raise ValueError("--model_path is required when --generate_script sample.py is used.")
        cmd = [
            args.python_exec,
            str(generate_script),
...
def parse_args():
    ...
    p.add_argument("--generate_script", type=str, default="sample.py")
    p.add_argument("--generated_eval_script", type=str, default="evaluate_generated_graphs.py")

    p.add_argument("--model_path", type=str, default=None,
                   help="FairWire checkpoint used by sample.py. Required for the default --generate_script sample.py.")
    ...
    p.add_argument("--sp_candidates", type=str, nargs="+",
                   default=["lp/score_sp_abs_gap_mean", "aggregate_lp/score_sp_abs_gap", "lp/sp_abs_gap_mean", "aggregate_lp/sp_abs_gap"])
...
def main():
    ...
    generate_script = Path(args.generate_script)
    if not generate_script.is_absolute():
        generate_script = repo_dir / generate_script
    if generate_script.name == "sample.py" and args.model_path is None:
        raise ValueError("--model_path is required when using the default FairWire sample.py generator.")
    if generate_script.name != "sample.py" and (args.run_name is None or args.checkpoint is None):
        raise ValueError("--run_name and --checkpoint are required when using an EDGE-style evaluate.py generator.")
    ...
    dataset_tag = sanitize_name(args.dataset)
    front_rows = pareto_front(agg_rows, x_key=x_key, y_key=y_key)
    write_csv(front_rows, out_dir / f"{dataset_tag}_pareto_front.csv")
    ...
    plot_pareto(
        ...
        png_path=out_dir / f"{dataset_tag}_pareto_auc_vs_sp.png",
        pdf_path=out_dir / f"{dataset_tag}_pareto_auc_vs_sp.pdf",
    )
