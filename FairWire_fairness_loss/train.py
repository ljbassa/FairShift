...
def main(args):
    ...
    if args.diffusion_T is not None:
        if args.diffusion_T < 1:
            raise ValueError("--diffusion_T must be >= 1.")
        yaml_data["diffusion"]["T"] = int(args.diffusion_T)
    ...
    if effective_stage == "auto":
        effective_stage = "fairwire" if args.alphaX is not None or args.alphaA is not None else "stage1"
    if effective_stage in ("stage1", "base"):
        alpha_X = 0.0
        alpha_A = 0.0
    else:
        alpha_X = args.alphaX if args.alphaX is not None else yaml_data["train"].get("fairness_mult_X", 0.0)
        alpha_A = args.alphaA if args.alphaA is not None else yaml_data["train"].get("fairness_mult_A", 0.0)
    print(f"[train] stage={effective_stage} | fairness_mult_X={alpha_X} | fairness_mult_A={alpha_A}")

    wandb.init(
        project=f"{args.dataset}-{model_name}",
        name=f"T{T}",
        config=config_df.to_dict(orient='records')[0])
    device = torch.device(args.device if args.device is not None else (f"cuda:{args.gpu}" if torch.cuda.is_available() else "cpu"))
    ...
    model_cpt_dir = f"{args.dataset}_{alpha_A}_{alpha_X}_cpts"
    ...
    for epoch in range(train_config["num_epochs"]):
        ...
        if to_save_cpt:
            ...
            torch.save({
                ...
                "diffusion_stage": effective_stage,
                "fairness_mult_X": alpha_X,
                "fairness_mult_A": alpha_A,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--stage", type=str, default="auto", choices=["auto", "stage1", "fairwire", "base"],
                        help="auto keeps old -aA/-aX behavior when multipliers are supplied; otherwise it runs stage1 with fair loss 0.")
    parser.add_argument("-aX", "--alphaX", type=float, default=None)
    parser.add_argument("-aA", "--alphaA", type=float, default=None)
    parser.add_argument("--diffusion_T", "--T", dest="diffusion_T", type=int, default=None,
                        help="Override configs/<dataset>/train_Sync.yaml diffusion.T for this run.")
