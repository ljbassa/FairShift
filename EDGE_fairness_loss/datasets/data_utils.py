...
def _get_passthrough_node_attrs(pyg_data, include_degree=False, augment_features=None):
    augment_features = set(augment_features or [])
    feats = {}
    for feat_name in pyg_data.keys():
        if feat_name in NODE_ATTR_SKIP_KEYS:
            continue
        if feat_name == 'degree' and not include_degree:
            continue
        if feat_name in augment_features:
            feats[feat_name] = getattr(pyg_data, feat_name)
            continue

        value = getattr(pyg_data, feat_name)
        if torch.is_tensor(value) and value.dim() >= 1 and value.size(0) == pyg_data.num_nodes:
...
def preprocess(g, degree=False, augmented_features=[]):
    ...
    if degree:
        # 고립 노드가 있어도 길이가 항상 pyg_data.num_nodes가 되도록 num_nodes 지정
        pyg_data.degree = pyg.utils.degree(
...
class EmpiricalEmptyGraphGenerator:
    def __init__(self, train_pyg_datas, degree=False, augment_features=[]):
        ...
        for pyg_data in train_pyg_datas:
            ...
            feats = _get_passthrough_node_attrs(
                pyg_data,
                include_degree=self.degree,
                augment_features=self.augment_features,
            )
            if self.degree and 'degree' not in feats:
                feats['degree'] = pyg.utils.degree(
                    pyg_data.edge_index[0], num_nodes=pyg_data.num_nodes
                )
...
class NeuralEmptyGraphGenerator:
    def __init__(self, train_pyg_datas, neural_attr_sampler, degree=False, device='cuda:0'):
        ...
        for pyg_data in train_pyg_datas:
            if pyg_data.num_nodes not in self.empirical_node_feat_dist:
                self.empirical_node_feat_dist[pyg_data.num_nodes] = []
            feats = _get_passthrough_node_attrs(pyg_data, include_degree=False)
            self.empirical_node_feat_dist[pyg_data.num_nodes].append(feats)
    ...
    def _sample_graph_size_and_features(self, num_samples):
        ...
        if self.degree:
            ...
            merged_feats = []
            for deg_hist, n_node in zip(xT_feats, ret):
                feats = random.choice(self.empirical_node_feat_dist[n_node]).copy()
                feats['degree'] = deg_hist_to_deg_seq(deg_hist)
                merged_feats.append(feats)
            xT_feats = merged_feats
        else:
            xT_feats = [
                random.choice(self.empirical_node_feat_dist[n_node]).copy()
                for n_node in sampled_sizes
            ]
            ret = sampled_sizes
        return ret, xT_feats
