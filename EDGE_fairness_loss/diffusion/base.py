...
class BaseExperiment(object):
    ...
    def compare_current_best(self, eval_dict):
        ...
        if self.current_best_eval is None:
            self.current_best_eval = np.nanmean([eval_dict[key] for key in self.monitoring_statistics])
            return True
        else:
            current_eval = np.nanmean([eval_dict[key] for key in self.monitoring_statistics])
...
class DataParallelDistribution(torch.nn.DataParallel):
    ...
    def scatter(self, inputs, kwargs, device_ids):
        ...
        if len(inputs) == 1:
            ...
            if isinstance(batch, Batch) and hasattr(batch, "to_data_list"):
                data_list = batch.to_data_list()
                n_dev = len(device_ids)

                q, r = divmod(len(data_list), n_dev)
                chunks = []
                start = 0
                for i in range(n_dev):
                    ...
                    if sz == 0:
                        continue
                    chunks.append(data_list[start:start+sz])
                    start += sz

                scattered_inputs = []
                scattered_kwargs = []

                for dev, chunk in zip(device_ids, chunks):
                    dev = torch.device("cuda", int(dev))

                    b = Batch.from_data_list(chunk).to(dev)

                    # ✅ 여기서 meta를 "해당 dev"에서 생성/부착 (이게 핵심)
                    nodes_per_graph = torch.tensor([d.num_nodes for d in chunk],
                                                dtype=torch.long, device=dev)
                    edges_per_graph = nodes_per_graph * (nodes_per_graph - 1) // 2

                    b.nodes_per_graph = nodes_per_graph
                    b.edges_per_graph = edges_per_graph

                    scattered_inputs.append((b,))
