...
class DiffusionBase(torch.nn.Module):
    ...
    def _sync_device_and_meta_(self, batched_graph):
        # 1) 입력으로부터 device 추론
        device = None
        for k in ("full_edge_index", "edge_index", "node_attr", "full_edge_attr", "batch"):
            if hasattr(batched_graph, k):
                v = getattr(batched_graph, k)
        ...
        # 2) replica마다 self.device를 현재 입력 device로 갱신 (DP에서 매우 중요)
        self.device = device

        # 3) repeat_interleave에 쓰는 meta 텐서들도 같은 device로 맞춤
        for k in ("nodes_per_graph", "edges_per_graph"):
            if hasattr(batched_graph, k):
                v = getattr(batched_graph, k)
                if torch.is_tensor(v) and v.device != device:
                    setattr(batched_graph, k, v.to(device))

        return device


    def forward(self, *args, mode=None, **kwargs):
        """
        DataParallel/Distributed wrapper compatibility.
        DataParallelDistribution.log_prob() calls forward(..., mode='log_prob').
        """
        if mode == 'log_prob':
            return self.log_prob(*args, **kwargs)
        elif mode == 'sample':
            return self.sample(*args, **kwargs)
        elif mode == 'sample_with_log_prob':
            return self.sample_with_log_prob(*args, **kwargs)
        else:
            raise TypeError(f"Unsupported mode={mode}. Expected one of "
                            f"['log_prob','sample','sample_with_log_prob'].")
    ...
    def p_sample(
        ...
        trace = {}  # base는 delta를 만들지 않으니 빈 trace
    ...
    def log_prob(self, batched_graph):
        self._sync_device_and_meta_(batched_graph)  # ✅ 추가
        if self.training:
    ...
    def sample(
        ...
        for t in reversed(range(0, self.num_timesteps)):
            ...
            # ✅ 이제 p_sample은 (node, edge, trace) 3개를 반환한다고 가정
            log_node_attr_tmin1, log_full_edge_attr_tmin1, trace = self.p_sample(
                ...
                delta_as_sparse_adj=delta_as_sparse_adj,
