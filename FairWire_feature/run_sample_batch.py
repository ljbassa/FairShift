...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Use Sync_T<T>.pth checkpoints. Overrides --checkpoint_glob.",
    )
    parser.add_argument(
        "--save_pkl_root",
        type=str,
...
def run_tag(dataset: str, alpha_a_text: str, alpha_x_text: str, seed: int, T: Optional[int] = None) -> str:
    t_part = "" if T is None else "_T{T}".format(T=T)
    return "{dataset}_aA_{alpha_a}_aX_{alpha_x}{t_part}_seed_{seed}".format(
        dataset=dataset,
...
def main() -> int:
    ...
    checkpoint_glob = "Sync_T{T}.pth".format(T=args.T) if args.T is not None else args.checkpoint_glob
    ...
    print("checkpoint : {glob}".format(glob=checkpoint_glob))
    ...
    for index, alpha_a in enumerate(args.alphaA_values, start=1):
        ...
        checkpoint_candidates = list_checkpoint_candidates(checkpoint_dir, checkpoint_glob)
        checkpoint_path, checkpoint_note = choose_checkpoint(checkpoint_candidates)
        run_name = run_tag(args.dataset, alpha_a_text, alpha_x_text, args.seed, args.T)
