...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Override diffusion.T in train.py. When set, --skip_existing only checks Sync_T<T>.pth.",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
...
def expected_checkpoint_path(checkpoint_dir: Path, T: Optional[int]) -> Optional[Path]:
    if T is None:
        return None
    return (checkpoint_dir / "Sync_T{T}.pth".format(T=T)).resolve()
...
def main() -> int:
    ...
    if args.T is not None:
        print("T override : {T}".format(T=args.T))
    ...
    for index, alpha_a in enumerate(args.alphaA_values, start=1):
        ...
        if args.T is None:
            ...
            checkpoint_path = expected_checkpoint_path(checkpoint_dir, args.T)
            checkpoint_note = ""
